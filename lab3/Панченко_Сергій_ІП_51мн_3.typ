...
- Розробити  програмний  скрипт,  що  реалізує  багатокритеріальне  оцінювання  ефективності маршрутів ВАШОГО переміщення із дому до місце навчання. Формування показників та критеріїв  ефективності,  синтез  багатокритеріальної  оптимізаційної  моделі  здійснити самостійно.

- Розробити  програмний  скрипт,  що  забезпечує  розв’язок  задачі  лінійного програмування для умов, зазначених в Лекції_6  з використанням інструментів бібліотеки Google  OR-Tools.  Порівняти  отримані  результати  із  графічним  методом  розв’язку,  що наведено  в  Лекції_6. Здійснити  опис  практичної  інтерпретації  задачі  лінійного програмування,  що  розв’язана  Вами  для  конкретної  прикладної  галузі  (із  власного практичного досвіду, або з аналізу інформаційних джерел). Опис подати у протоколі.  Приклади реалізації завдань див. матеріали Лекцій 6,7. 
...
У результаті виконання лабораторної роботи було успішно розроблено макет інтелектуальної ERP-системи підтримки прийняття рішень на базі алгоритмів багатокритеріального аналізу (Multicriteria Decision Analysis — MCDA).

На першому етапі (Рівень І) було формалізовано та розв'язано задачу вибору об'єкта нерухомості серед 8 альтернатив за 12 різнорозмірними критеріями. На другому етапі (Рівень ІІ) систему було адаптовано до реальних картографічних даних для розв'язання логістичної задачі — оцінювання 11 маршрутів переміщення до місця навчання з урахуванням часу, вартості, пересадок та показників комфорту.

Дослідження підтвердило, що використання методу мінімаксної нормалізації дозволяє коректно звести різнотипні показники (ті, що максимізуються, і ті, що мінімізуються) до єдиної безрозмірної шкали $[0; 1]$. Подальше застосування адитивної згортки (зваженої суми) дає змогу отримати об'єктивну інтегральну оцінку для кожної альтернативи.